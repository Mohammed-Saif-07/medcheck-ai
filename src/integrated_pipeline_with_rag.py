"""
MedCheck - COMPLETE INTEGRATED PIPELINE with RAG
Connects: OCR → ML → RAG → Safety Report

Production-ready system with evidence and citations!
"""
...
class MedCheckPipelineWithRAG:
    """
    Complete integrated pipeline with RAG:
    Image → OCR → ML Prediction → RAG Evidence → Safety Report
    """
    
    def __init__(self):
        print("="*70)
        print("💊 MEDCHECK - COMPLETE AI PIPELINE WITH RAG")
        print("="*70)
        print()
        ...
        if not self.rag.documents:
            print("   ⚠️  No RAG database found")
            print("   ℹ️  Will work without evidence citations")
            self.use_rag = False
        else:
            self.use_rag = True
    ...
    def process_prescription(self, image_path: str) -> dict:
        """
        Main pipeline: Process prescription image end-to-end with RAG
        """
        print("="*70)
            ...
            for drug1, drug2 in combinations(drug_names, 2):
                ...
                if not self.use_fallback:
                    ...
                    if severity != 'NONE':
                        ...
                        print(f"      ✅ No interaction predicted")
                else:
        ...
        if interactions and self.use_rag:
            ...
            for interaction in interactions:
                print(f"   Retrieving evidence for: {interaction['drug1'].upper()} + {interaction['drug2'].upper()}")
                
                # Get evidence from RAG
                ...
                if evidence['evidence_found']:
                    print(f"      ✅ Found {len(evidence['sources'])} evidence sources")
                else:
                    print(f"      ℹ️  Limited evidence available")
            
            print()
            print("✅ Stage 3 Complete: Evidence retrieved")
            print()
    ...
    def _get_recommendations(self, risk_level: str, interactions: list) -> list:
        ...
        # Add specific recommendations for high-risk interactions
        for interaction in interactions:
    ...
    def generate_report(self, result: dict):
        """Generate comprehensive safety report with RAG evidence"""
        
        print("="*70)
        print("📄 MEDICATION SAFETY REPORT WITH EVIDENCE")
        print("="*70)
        print()
    ...
    def _save_report(self, result: dict):
        ...
        report_file = results_dir / f"safety_report_with_rag_{timestamp}.json"
...
def main():
    """Demo the complete pipeline with RAG"""
    
    print("\n" + "="*70)
    print("💊 MedCheck - Complete AI Pipeline with RAG Demo")
    print("="*70)
    print()
    print("This demonstrates the FULL system with RAG:")
    print("  1. Advanced OCR (Computer Vision)")
    print("  2. ML Interaction Prediction")
    print("  3. RAG Evidence Retrieval")
    print("  4. Risk Assessment")
    print("  5. Comprehensive Safety Report")
    print()
    print("="*70)
    print()
    
    # Initialize pipeline
    pipeline = MedCheckPipelineWithRAG()
    ...
    print("="*70)
    print("✅ COMPLETE PIPELINE WITH RAG EXECUTED!")
    print("="*70)
    print()
    print("🎯 What just happened:")
    print("   1. ✅ OCR extracted drug names from image")
    print("   2. ✅ ML model predicted interaction risks")
    print("   3. ✅ RAG retrieved evidence from FDA database")
    print("   4. ✅ Risk assessment calculated")
    print("   5. ✅ Comprehensive report with citations generated")
    print()
    print("💼 Resume Value:")
    print("   'Built complete AI pipeline integrating Computer Vision,")
    print("   Machine Learning, and RAG (Retrieval Augmented Generation)")
    print("   with evidence-based citations from FDA database'")
    print()
